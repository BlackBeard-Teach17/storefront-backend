...
export class OrderStore {
    ...
    async checkOrderStatus(order_id: string): Promise<Boolean> {
        try {
            const conn = await client.connect();
            const sql = 'SELECT status FROM orders WHERE order_id=($1)';
    ...
    async checkOrderProductExists(order_id: string, product_id: string, quantity: number): Promise<Boolean> {
        try {
            const conn = await client.connect();
            ...
            if(result.rows[0].product_id === product_id){
                ...
                await this.updateOrderProduct(parseInt(order_id), parseInt(product_id), new_quantity);
                return true;
            }
            return false;
        } catch (err) {
    ...
    async addToCart(order_id: number, product_id: number, quantity: number): Promise<Order | string> {
        if (await this.checkIfOrderExists(order_id.toString()) === false)
            return 'Order does not exist';
        if (await this.checkOrderStatus(order_id.toString()) === false)
            return 'Order is completed';
        if (await this.checkOrderProductExists(order_id.toString(), product_id.toString(), quantity) === true)
    ...
    async addProduct(order_id: number, product_id: number, quantity: number): Promise<Order | string> {
        try {
            const conn = await client.connect();
    ...
    async removeProduct(order_id: number, product_id: number): Promise<Order | string> {
        if (await this.checkOrderStatus(order_id.toString()) === false)
            return 'Order is completed';
        try {
            const conn = await client.connect();
